...
class CandidateController extends BaseController
{
    ...
    public function jobBookmarks(Request $request, PaginatorInterface $paginator, BookmarkService $bookmarkService)
    {
        $bookmarks = $paginator->paginate(
            $bookmarkService->getJobBookmarks($this->getUser()),
            $request->query->getInt('page', 1)
        );
    ...
    #[Route('/job-bookmarks/{id}/delete', name: 'job_bookmarks_delete')]
    public function deleteJobBookmark(Request $request, JobBookmark $jobBookmark, EntityManagerInterface $manager)
    {
        if ($this->isCsrfTokenValid('delete'.$jobBookmark->getId(), $request->request->get('_token'))) {
            $manager->remove($jobBookmark);
            $manager->flush();

            $this->addFlash('success', 'ui.alerts.job_bookmark_deleted');

            return $this->redirectBack('app_front_candidate_job_bookmarks');
        }
    }
    ...
    public function toggleJobBookmark(string $slug, int $id, BookmarkService $bookmarkService)
    {
        ...
        $jobBookmark->setUser($this->getUser());
    }
}
